...
async def br_updatedeck(interaction, deck_csv):
    """
    ...
    deck_csv : discord.Attachment
        ...
        headings:
            ...
            difficulty - integer (evaluation of the card's impact on flagstring difficulty, from -2 [easiest] to +2 [hardest])
    ...
    async with asqlite.connect(path) as conn:
        async with conn.cursor() as cursor:
            await cursor.execute("DROP TABLE br_cards")
            print('dropped table')
