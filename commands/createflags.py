...
async def createflags(interaction, cards) -> str:
    ...
    """
    obj_count = 0
    obj_prefix = ['oa','ob','oc','od','oe','of','og','oh','oi','oj','ok','ol','om','on','oo','op','oq','or','os','ot',
                  'ou','ov','ow','ox','oy','oz']
    ...
    ktcard = await last_card(cards,50,51,52,53)
    ktchars = 6
    ktespers = 9
    ktdragons = 0
    ktbosses = 0
    if ktcard == 50:
        ktchars = 8
        ktespers = 12
        kt = '.'.join([' -' + obj_prefix[obj_count] + ' 2.2.2.2', str(ktchars), str(ktchars), '4', str(ktespers), str(ktespers)])
        obj_count += 1
    elif ktcard == 51:
        ktchars = 5
        ktespers = 8
    ...
    if skipcard == 54:
        kt += '.'.join([' -' + obj_prefix[obj_count] + ' 3.1.1.2', str(ktchars+3), str(ktchars+3), '4', str(ktespers+3), str(ktespers+3)])
        obj_count += 1
    elif skipcard == 55:
        ...
        kt += '.'.join([' -' + obj_prefix[obj_count] + ' 3.1.1.10', str(ktespers + ktchars + ktdragons + 5), str(ktespers + ktchars + ktdragons + 5)])
        obj_count += 1
    elif skipcard == 59:
    ...
    print(f'after removing and downsizing to party size cap, the party-related cards in this one are {party_cards}')
    ...
    '''
    scale_card = await last_card(cards,43,44,45,46,47)
    if scale_card == 43:
        lscale = ' -lsced 4'
        hmscale = ' -hmced 1'
        xgscale = ' -xgced 2.5'
    ...
    esper_card = await last_card(cards,79,80)
    if esper_card == 79:
        esr1 = 3
        esr2 = 5
        ebonus = ' -ebr 100'
    ...
    # SHOPS
    shop_card = await last_card(cards,68,69,70,71,133)
    if shop_card == 68:
        shopinv = ' -sirt'
        sprp1 = 50
        sprp2 = 150
    ...
    if 114 in cards:
        ...
        pd = ''
    challenges = ''.join([nmc, noshoes, nee, nil, nfps, nu, nfp, pd])
